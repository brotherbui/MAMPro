...
if [ "$phpversion" != "" ]; then
  if [ ! -f "$xdebug_conf" ] && [ ! -f "$xdebug_conf.disabled" ]; then
    ...
    if [ -f "$xdebug_conf" ] && [ -f "$xdebug_conf.disabled" ]; then
      echo "$Red"
      echo "Detected both enabled and disabled Xdebug ini files. Deleting the disabled one. $Color_Off"
...
if [ "$SHOWHELP" == "yes" ]; then
  ...
  if [ "$mod_php" != "" ]; then
    ...
    echo "Example: x e 8.0 $Color_Off"
  fi
...
if [ "$RESTART" == "yes" ]; then
  echo "$Green"
  echo "Xdebug has been $STATUS, restarting server $Color_Off"
  if [ "$mod_php" != "" ]; then
